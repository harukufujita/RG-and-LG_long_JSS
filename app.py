# ------------------------------------------------------------
# 3-year RFS & OS Predictor (RSF models)
# ------------------------------------------------------------
...
rsf_rfs = load_model("rsf_final_15vars.pkl")       # RFS model
rsf_os  = load_model("rsf_final_os_16vars.pkl")    # OS model
...
dissection_map = {"D0": 0, "D1": 1, "D1+": 2, "D2": 3}
...
# ────────────────────────────────────────────────────────────
st.markdown(
    "<h3 style='text-align:center;margin-bottom:1rem;'>3-year RFS & OS Predictor for GC</h3>",
...
bmi_str = st.text_input(
    ...
    key="bmi",
    help="Press the button above to calculate BMI automatically, or enter it directly."
)
...
surg = st.selectbox(
    ...
    list(surg_map.keys()),
    index=None,
    placeholder="Select surgical method"
)
...
recon = st.selectbox(
    ...
    recon_options,
    index=None,
    placeholder="Select reconstruction"
)

# Extent of lymphadenectomy: one below Reconstruction
dissection = st.selectbox(
    "Extent of lymphadenectomy",
    list(dissection_map.keys()),
    index=None,
    placeholder="Select extent"
)

# Postoperative complication: one below Reconstruction -> now below dissection
cd3a = st.selectbox(
    "Postoperative complication (CD≥IIIa)",
    list(cd3a_map.keys()),
    index=None,
    placeholder="Select postoperative complication"
)
...
macro = st.selectbox(
    ...
    list(macro_map.keys()),
    index=None,
    placeholder="Select macroscopic type"
)
...
vcat = st.selectbox(
    ...
    list(v_map.keys()),
    index=None,
    placeholder="Select vascular invasion"
)

# Lymphatic invasion: one below Vascular invasion
lycat = st.selectbox(
    "Lymphatic invasion (ly)",
    list(ly_map.keys()),
    index=None,
    placeholder="Select lymphatic invasion"
)
...
stage = st.selectbox(
    ...
    help="Auto-calculated from pT and pN when available. You may also select IV (CY1)."
)
...
if st.button("Predict"):
    ...
    required_fields = [
        sex, asa, surg, recon, dissection, cd3a,
        macro, histo, vcat, lycat, pt, pn, stage
    ]
    if any(x is None for x in required_fields):
        st.error("Please complete all categorical selections before prediction.")
        st.stop()
    ...
    inp = pd.DataFrame([{
        ...
        "dissection2": dissection_map[dissection],
        "CD3a": cd3a_map[cd3a],
    ...
    try:
        ...
        st.write("RFS model features:", getattr(rsf_rfs, "feature_names_in_", "Not available"))
        st.write("OS model features:", getattr(rsf_os, "feature_names_in_", "Not available"))
        st.stop()

    try:
        surv_funcs_rfs = rsf_rfs.predict_survival_function(inp_rfs)
        surv_funcs_os  = rsf_os.predict_survival_function(inp_os)

        time_grid = np.arange(0, 37)

        surv_rfs = np.mean(
            [np.interp(time_grid, fn.x, fn.y) for fn in surv_funcs_rfs],
            axis=0
        )
        surv_os = np.mean(
            [np.interp(time_grid, fn.x, fn.y) for fn in surv_funcs_os],
            axis=0
        )
